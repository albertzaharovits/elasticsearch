...
public class RestSudoGetStoredScriptAction extends BaseRestHandler {
    ...
    public RestSudoGetStoredScriptAction(Settings settings, RestController controller) {
        super(settings);

        controller.registerHandler(GET, "/sudo/_scripts/{id}", this);
    }

    @Override
    public String getName() {
        return "sudo_get_stored_scripts_action";
    }

    @Override
    public RestChannelConsumer prepareRequest(final RestRequest request, NodeClient client) throws IOException {
        String id = request.param("id");
        SudoGetStoredScriptRequest sudoGetRequest = new SudoGetStoredScriptRequest(id);

        return channel -> client.execute(SudoGetStoredScriptAction.INSTANCE, sudoGetRequest,
                new RestBuilderListener<SudoGetStoredScriptResponse>(channel) {
            @Override
            public RestResponse buildResponse(SudoGetStoredScriptResponse response, XContentBuilder builder) throws Exception {
                builder.startObject();
